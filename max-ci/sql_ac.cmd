...
ECHO.
rem "D:\xampp\mysql\bin\mysql.exe" --user=root --password= --default-character-set=utf8  chromatin -e "DROP DATABASE chromatin;CREATE DATABASE `chromatin` /*!40100 COLLATE 'utf8_general_ci' */;"

color 3
ECHO ----------------------sql geri yukleme yapılıyor ------------------------- 
rem "D:\xampp\mysql\bin\mysql.exe" --user=root --password= --default-character-set=utf8  chromatin < app.sql


color 5
ECHO ----------------------auto increment fix------------------------- 
php -f yedekler/autoincrement-fix/index.php -- -s run
...
rem iptal edildi 
echo -------------işlem tamam-------------------------------
rem "D:\xampp\mysql\bin\mysqldump.exe" --user=root --password=123456 --skip-opt chromatin > diger/%TODAY%.sql"
