...
if (! function_exists ( 'cubrid_connect' ))
	die ( '<b>Fatal Error:</b> ezSQL_cubrid requires CUBRID PHP Driver to be compiled and or linked in to the PHP engine' );
if (! class_exists ( 'ezSQLcore' ))
	die ( '<b>Fatal Error:</b> ezSQL_cubrid requires ezSQLcore (ez_sql_core.php) to be included/loaded before it can be used' );
class ezSQL_cubrid extends ezSQLcore {
	 ...
	 * because std.
	 * connect already does what quick connect does -
	 ...
	 * i.e.
	 * Oracle: SYSDATE Mysql/CUBRID: NOW()
	 */
	function sysdate() {
		return 'NOW()';
	}
